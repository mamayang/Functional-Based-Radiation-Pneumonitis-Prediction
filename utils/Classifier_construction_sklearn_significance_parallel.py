...
def scores_extract_metrics_with_estimator(scores=None,model_name=None,modality_name=None,reduction_method = None):
    ...
    if opt.need_classifier_optimization:
        model_dir = os.path.join('./Parallel/V2/','model_save',modality, 'with_ml_classification','after_significance_selection',opt.test_name,reduction_method)
        if not os.path.exists(model_dir ):
            os.makedirs(model_dir)
    else:
        model_dir  = os.path.join('./Parallel/V2','model_save',modality, 'without_ml_classification','after_significance_selection',opt.test_name,reduction_method)
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
