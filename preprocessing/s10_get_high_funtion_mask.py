    ...
    for subdir, dirs, files in os.walk(input_base_directory):
        for file in files:
            if file == "Ven_1500_7.mha":
                ...
                # 计算值域的前30%和0.1%的阈值
                lower_threshold = array.max() * opt.function_threshold
                upper_threshold = array.max() * 1.0
                ...
                output_path = os.path.join(output_subdir, "high_ventilation_mask.mha")
            ...
            if file == "moved_back_perfusion.nii":
                ...
                output_subdir = os.path.join(output_base_directory, last_part)
                os.makedirs(output_subdir, exist_ok=True)  # 创建输出目录（如果不存在）
                output_path = os.path.join(output_subdir, "high_perfusion_mask.mha")
...
# output_base_directory = os.path.join('../dataset/for_radiomics/ /CT_origin',str(opt.function_threshold))
